...
Name: "replacerflauncher"; Description: "Replace the Red Faction launcher with the Dash Faction launcher (allows Dash Faction to be used with Steam)"; GroupDescription: "Other options:"; Flags: unchecked
...
[CustomMessages]
RFExeLocation=Setup will attempt to locate the RF.exe file automatically. It may fail in certain cases, so please make sure the location is correct.%n%nTo continue, click Next. To select a different file, click Browse.
GameNeedsPatches=Your game version is not directly supported by Dash Faction. Setup will install the necessary patches automatically.%n%nPatches that will be installed:%n
UnkGameExeVersion=Unknown RF.exe version.%nPlease make sure the location points to an official RF.exe file, or Dash Faction may not function.%n%nYou can find help at:%n> https://discord.gg/factionfiles%n> https://redfaction.help%n%nTechnical details:%nSHA1 = %1%n%nDo you want to ignore this error and continue?
...
begin
    SelectGameExePage := CreateInputFilePage(wpSelectDir, 'Select RF.exe Location', 'This file can be found in the Red Faction game folder.', ExpandConstant('{cm:RFExeLocation}'));
