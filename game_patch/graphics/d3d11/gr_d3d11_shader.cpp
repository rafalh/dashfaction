...
D3D11ShaderManager::D3D11ShaderManager(ComPtr<ID3D11Device> device) : device_{device}
{
    init_default_shaders();
    init_character_shaders();
}

void D3D11ShaderManager::bind_default_shaders(D3D11RenderContext& render_context)
{
...
void D3D11ShaderManager::bind_character_shaders(D3D11RenderContext& render_context)
{
...
std::pair<ComPtr<ID3D11VertexShader>, ComPtr<ID3D11InputLayout>> D3D11ShaderManager::load_vertex_shader(const char* filename, const D3D11_INPUT_ELEMENT_DESC input_elements[], std::size_t num_input_elements)
{
    rf::File file;
...
ComPtr<ID3D11PixelShader> D3D11ShaderManager::load_pixel_shader(const char* filename)
{
    rf::File file;
    if (file.open(filename) != 0) {
...
void D3D11ShaderManager::init_default_shaders()
{
...
void D3D11ShaderManager::init_character_shaders()
{
    D3D11_INPUT_ELEMENT_DESC desc[] = {
        { "POSITION", 0, DXGI_FORMAT_R32G32B32_FLOAT, 0, D3D11_APPEND_ALIGNED_ELEMENT, D3D11_INPUT_PER_VERTEX_DATA, 0 },
        // { "NORMAL", 0, DXGI_FORMAT_R32G32B32_FLOAT, 0, D3D11_APPEND_ALIGNED_ELEMENT, D3D11_INPUT_PER_VERTEX_DATA, 0 },
