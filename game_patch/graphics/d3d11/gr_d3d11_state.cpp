...
{
    StateManager::StateManager(ComPtr<ID3D11Device> device) : device_{std::move(device)} {}

    ComPtr<ID3D11RasterizerState>
    StateManager::create_rasterizer_state(D3D11_CULL_MODE cull_mode, int depth_bias, bool depth_clip_enable)
    {
        ...
        check_hr(device_->CreateRasterizerState(&desc, &rasterizer_state), [=]() {
    ...
    ComPtr<ID3D11SamplerState> StateManager::create_sampler_state(rf::gr::TextureSource ts)
    {
        ...
        check_hr(device_->CreateSamplerState(&desc, &sampler_state), [=]() {
    ...
    ComPtr<ID3D11BlendState> StateManager::create_blend_state(gr::AlphaBlend ab)
    {
        ...
        check_hr(device_->CreateBlendState(&desc, &blend_state), [=]() {
    ...
    ComPtr<ID3D11DepthStencilState> StateManager::create_depth_stencil_state(gr::ZbufferType zbt)
    {
        ...
        check_hr(device_->CreateDepthStencilState(&desc, &depth_stencil_state), [=]() {
