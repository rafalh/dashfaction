...
void RoomRenderCache::render(FaceRenderType render_type, ID3D11Device* device, D3D11RenderContext& context)
{
    if (invalid()) {
...
void D3D11SolidRenderer::render_dynamic_decal(GDecal* decal, GRoom* room)
{
...
void D3D11SolidRenderer::render_dynamic_decals(rf::GRoom** rooms, int num_rooms)
{
    int num_rendered_decals = 0;
    // TODO: try using D3D11_RASTERIZER_DESC::DepthBias
    // gr::d3d::zbias = gr::d3d::matrix_scale.z * 0.025f;
...
void D3D11SolidRenderer::render_room_faces(rf::GSolid* solid, rf::GRoom* room, FaceRenderType render_type)
{
...
void D3D11SolidRenderer::render_detail([[maybe_unused]] rf::GSolid* solid, [[maybe_unused]] GRoom* room, [[maybe_unused]] bool alpha)
{
...
void D3D11SolidRenderer::clear_cache()
{
...
void D3D11SolidRenderer::render_sky_room(GRoom *room)
{
...
void D3D11SolidRenderer::render_movable_solid(GSolid* solid, const Vector3& pos, const Matrix3& orient)
{
    if (!solid->field_370) {
...
void D3D11SolidRenderer::render_alpha_detail(GRoom *room, GSolid *solid)
{
...
void D3D11SolidRenderer::render_room_liquid_surface(GSolid* solid, GRoom* room)
{
    before_render(rf::zero_vector, rf::identity_matrix, false);
...
D3D11SolidRenderer::D3D11SolidRenderer(ComPtr<ID3D11Device> device, ComPtr<ID3D11DeviceContext> context, D3D11RenderContext& render_context) :
    device_{std::move(device)}, context_{std::move(context)}, render_context_(render_context)
{
}

void D3D11SolidRenderer::render_solid(rf::GSolid* solid, rf::GRoom** rooms, int num_rooms)
{
    before_render(rf::zero_vector, rf::identity_matrix, false);
