...
BOOL OptionsDlg::OnInitDialog()
{
    ...
    CheckDlgButton(IDC_DERANDOMIZE_BOMB, m_conf.derandomize_bomb ? BST_CHECKED : BST_UNCHECKED);
...
void OptionsDlg::InitToolTip()
{
    ...
    m_tool_tip.AddTool(GetDlgItem(IDC_DERANDOMIZE_BOMB), "Enables a static solution to the bomb which persists across retries. Recommended for first time players");
}
...
void OptionsDlg::OnBnClickedOk()
{
    ...
    m_conf.derandomize_bomb = (IsDlgButtonChecked(IDC_DERANDOMIZE_BOMB) == BST_CHECKED);
