...
bool LauncherApp::LaunchGame(HWND hwnd, const char* mod_name)
{
    ...
    try {
        ...
        ss << "Game directory validation has failed! File is missing:\n"
           << e.get_file_name() << "\n"
           << "Please make sure game executable specified in options is located inside a valid Red Faction "
              "installation "
           << "root directory.";
        ...
        if (e.get_file_name() == "tables.vpp") {
            ss << "\n\nIt can prevent multiplayer functionality or entire game from working properly.\n"
               ...
               << "replace your tables.vpp file with original 1.20 NA " << e.get_file_name()
               << " available on FactionFiles.com.\n"
    ...
    try {
        ...
        Message(
            hwnd, "Game executable has not been found. Please set a proper path in Options.", nullptr,
            MB_OK | MB_ICONERROR
        );
    }
    catch (FileHashVerificationException& e) {
        ...
        if (Message(hwnd, str.c_str(), nullptr, MB_OKCANCEL | MB_ICONERROR) == IDOK) {
            ShellExecuteA(
                hwnd, "open", "https://www.factionfiles.com/ff.php?action=file&id=517545", nullptr, nullptr, SW_SHOW
            );
        }
    }
